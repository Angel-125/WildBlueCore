...
namespace WildBlueCore
{
    ...
    public class ModulePartGridVariants : BasePartModule, IPartCostModifier, IPartMassModifier, IModuleInfo
    {
        ...
        public override void OnStart(StartState state)
        {
            ...
            // Setup variants
            setupVariantSelector("rowIndex", "#LOC_WILDBLUECORE_rowsVariant", totalRows, "#000000", "#ffffff");
            setupVariantSelector("columnIndex", "#LOC_WILDBLUECORE_columnsVariant", totalColumns, "#ffffff", "#000000");
        ...
        {
            return Localizer.Format("#LOC_WILDBLUECORE_partGridVariantsModuleInfo", new string[2] { totalRows.ToString(), totalColumns.ToString() });
        }

        public override string GetModuleDisplayName()
        {
            return Localizer.Format("#LOC_WILDBLUECORE_partGridVariantsModuleName");
        }
        #endregion
        ...
        {
            return Localizer.Format("#LOC_WILDBLUECORE_partGridVariantsModulePrimaryInfo", new string[2] { totalRows.ToString(), totalColumns.ToString() });
        }
        ...
        private void findGridElements()
        {
            ...
            // New way to get transforms: they're all named with the same name.
            gridElements = new List<Transform>();
            Transform gridElement;
            Transform[] transforms = part.FindModelTransforms(elementTransformName);
            if (transforms.Length > 0)
            {
                totalElements = transforms.Length;
                for (int index = 0; index < transforms.Length; index++)
                {
                    gridElement = transforms[index];
                    gridElements.Add(gridElement);
                    setEnabled(gridElement);
                }
            }

            // Old way: each element has an identifier suffix like foo001.
            else
            {
