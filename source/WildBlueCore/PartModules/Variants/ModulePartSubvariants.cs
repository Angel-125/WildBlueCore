...
namespace WildBlueCore.PartModules
{
    ...
    /// gets around the issue and adds a few enhancements. When you define a ModulePartVariants, be sure to place its config node AFTER ModulePartVariants.  
    ...
    public class ModulePartSubvariants : BasePartModule, IPartCostModifier, IPartMassModifier
    {
        ...
        public override void OnStart(StartState state)
        {
            ...
            if (!string.IsNullOrEmpty(currentMeshSet))
                variantApplied = true;
            else if (!string.IsNullOrEmpty(baseVariant) && !variantApplied && variants!= null && variants.Count > 0)
            {
                int count = variants.Count;
                for (int index = 0; index < count; index++)
                {
                    if (variants[index].name == baseVariant)
                    {
                        variantApplied = true;
                        variantIndex = index;
                        break;
        ...
        public override void OnAwake()
        {
            ...
            onApplySubvariantExtraInfo.Add(onApplyVariantExtraInfoHandler);
        ...
        public void OnDestroy()
        {
            ...
            onApplySubvariantExtraInfo.Remove(onApplyVariantExtraInfoHandler);
        }
        ...
        public void applyVariant(int newIndex, bool fireEvents = false)
        {
            ...
            WBCVariant variant = variants[variantIndex];
            ...
            if (fireEvents)
            {
                ...
                */
                ...
                if (variant.extraInfo.Count > 0)
                    onApplySubvariantExtraInfo.Fire(this, variant.name, variant.extraInfo);
            }
